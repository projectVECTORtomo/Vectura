 ...
 * Part of Project VECTOR by projectVECTORtomo
 */
...
// User-Specific Vector Bold Formatting
...
function syncState() {
    ...
    if (!raw) {
        els.colControls.innerHTML = `<div class="text-center py-20 text-slate-300 font-bold italic text-base">Awaiting data input...</div>`;
        return;
    }
        ...
        <div class="column-card p-8 rounded-[2rem] ${col.isManual ? 'manual-edit shadow-indigo-100 shadow-xl' : 'auto-detected shadow-md'}">
            ...
            <div class="grid grid-cols-2 gap-4 mb-6">
                <div class="space-y-1">
                    ...
                    <input type="text" value="${col.name}" class="name-input w-full bg-slate-50 border-2 border-slate-100 p-3 rounded-xl text-xs font-bold" data-idx="${i}">
                </div>
                <div class="space-y-1">
                    <label class="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-2 block">Unit</label>
                    <input type="text" value="${col.unit}" class="unit-input w-full bg-slate-50 border-2 border-slate-100 p-3 rounded-xl text-xs text-indigo-600 font-black" placeholder="V, A..." data-idx="${i}">
                </div>
            </div>
...
function generate() {
    const raw = els.input.value.trim();
    if (!raw) { els.output.textContent = "% Vectura Engine Ready. Awaiting data..."; return; }
    const lines = raw.split('\n').filter(l => l.trim() !== '');
    ...
    let latex = `% Generated by Vectura (Project VECTOR)\n\\begin{table}[htbp]\n  \\centering\n`;
    ...
    latex += `  \\end{tabular}\n`;
    if (autoResize) latex += `  }\n`;
    latex += `  \\label{${normalizeText(els.label.value).replace(/\s+/g, '_') || "tab:measured_data"}}\n\\end{table}`;
...
els.sigFigs.addEventListener('input', e => { els.sigFigLabel.textContent = `${e.target.value} Digits`; generate(); });
